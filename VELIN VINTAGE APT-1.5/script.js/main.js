...
function renderProducts(items) {
    const html = items.map(p => {
        ...
        const trend = getPriceTrend(p.id);

        return `
                <div class="product-card" onclick="window.location.href='product.html?id=${p.id}'">
                    <div class="product-image">
                        ${p.badge ? `<div class="product-badge">${p.badge}</div>` : ''}
                        ${p.image ? `<img src="${p.image}" alt="${p.name}">` : `<div style="color: #666; font-size: 14px; font-weight: 700; letter-spacing: 3px;">${p.category.toUpperCase()}</div>`}
                    </div>
                    <div class="product-info">
                        ...
                        <div class="product-price">
                            $${currentPrice}
                            ${p.original ? `<span class="product-price-original">$${p.original}</span>` : ''}
                        </div>
                        <div class="price-trend ${trend >= 0 ? 'up' : 'down'}">
...
function updateFilterButtons() {
    ...
    buttons.forEach((btn, i) => {
        btn.classList.toggle('active', categories[i] === currentFilter);
        btn.onclick = () => filterProducts(categories[i]);
    });
}

function renderMarketTrends() {
    const marketItems = [
        { id: 1, name: 'CHROME HEARTS AVG', productId: 1 },
        { id: 2, name: 'BAPE CAMO SERIES', productId: 2 },
        { id: 6, name: 'SUPREME BOX LOGO', productId: 6 },
        { id: 7, name: 'OFF-WHITE ARROWS', productId: 7 }
    ];

    const html = marketItems.map(item => {
        const currentPrice = getCurrentPrice(item.productId);
        const trend = getPriceTrend(item.productId);

        return `
                <div class="market-card" onclick="window.location.href='product.html?id=${item.productId}'">
                    <h3>${item.name}</h3>
                    <div class="market-price">$${currentPrice}</div>
                    <div class="market-change ${trend >= 0 ? 'up' : 'down'}">
                        ${trend >= 0 ? '↑' : '↓'} ${Math.abs(trend)}% this week
                    </div>
                </div>
            `;
